...
public class FastBoard {
    ...
    static {
        try {
            if (fr.cite.core.scoreboard.FastReflection.nmsOptionalClass("ScoreboardServer$Action").isPresent()) {
                VERSION_TYPE = VersionType.V1_13;
            } else if (fr.cite.core.scoreboard.FastReflection.nmsOptionalClass("IScoreboardCriteria$EnumScoreboardHealthDisplay").isPresent()) {
                VERSION_TYPE = VersionType.V1_8;
            } else {
                VERSION_TYPE = VersionType.V1_7;
            }

            Class<?> craftChatMessageClass = fr.cite.core.scoreboard.FastReflection.obcClass("util.CraftChatMessage");
            Class<?> entityPlayerClass = fr.cite.core.scoreboard.FastReflection.nmsClass("EntityPlayer");
            Class<?> playerConnectionClass = fr.cite.core.scoreboard.FastReflection.nmsClass("PlayerConnection");
            Class<?> craftPlayerClass = fr.cite.core.scoreboard.FastReflection.obcClass("entity.CraftPlayer");

            MESSAGE_FROM_STRING = craftChatMessageClass.getDeclaredMethod("fromString", String.class);
            CHAT_COMPONENT_CLASS = fr.cite.core.scoreboard.FastReflection.nmsClass("IChatBaseComponent");
            ...
            SEND_PACKET = playerConnectionClass.getDeclaredMethod("sendPacket", fr.cite.core.scoreboard.FastReflection.nmsClass("Packet"));

            PACKET_SB_OBJ =fr.cite.core.scoreboard.FastReflection.nmsClass("PacketPlayOutScoreboardObjective").getConstructor();
            PACKET_SB_DISPLAY_OBJ = fr.cite.core.scoreboard.FastReflection.nmsClass("PacketPlayOutScoreboardDisplayObjective").getConstructor();
            PACKET_SB_SCORE = fr.cite.core.scoreboard.FastReflection.nmsClass("PacketPlayOutScoreboardScore").getConstructor();
            PACKET_SB_TEAM = fr.cite.core.scoreboard.FastReflection.nmsClass("PacketPlayOutScoreboardTeam").getConstructor();

            if (VersionType.V1_8.isHigherOrEqual()) {
                ENUM_SB_HEALTH_DISPLAY = fr.cite.core.scoreboard.FastReflection.nmsClass("IScoreboardCriteria$EnumScoreboardHealthDisplay");

                if (VersionType.V1_13.isHigherOrEqual()) {
                    ENUM_SB_ACTION = fr.cite.core.scoreboard.FastReflection.nmsClass("ScoreboardServer$Action");
                } else {
                    ENUM_SB_ACTION = fr.cite.core.scoreboard.FastReflection.nmsClass("PacketPlayOutScoreboardScore$EnumScoreboardAction");
                }

                ENUM_SB_HEALTH_DISPLAY_INTEGER = fr.cite.core.scoreboard.FastReflection.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "INTEGER");
                ENUM_SB_ACTION_CHANGE = fr.cite.core.scoreboard.FastReflection.enumValueOf(ENUM_SB_ACTION, "CHANGE");
                ENUM_SB_ACTION_REMOVE = fr.cite.core.scoreboard.FastReflection.enumValueOf(ENUM_SB_ACTION, "REMOVE");
            } else {
